...
setwd("C:/Users/Fangwei Cheng/Dropbox/LCNC_revise/Cheng_LCNC")
...
gwp_electricity <- 450/1000 # kg CO2/kwh
gwp_NG <- 0.066  # kg CO2/MJ
gwp_hexane <- 35.35/1000 #kg CO2/kg
gwp_h2 <- 11070/1000 #kg CO2/kg
...
HTT_Action_GWP <- function(a){
  ...
  # calculate the GWP associated with fossil fuel energy consumption
...
Pyrolysis_Action_GWP <- function(a){
  ...
  # get the energy content in oil and gas; oil and gas were combusted to provide heat, with efficiency of 85%
...
library(ggplot2)
plot_GWP <- ggplot() + geom_bar(data = Action_summary, aes(x = Action, y = GWP_mean, fill = technology),stat="identity")+
  ...
  ggtitle("(a). GWP of each action, electricity and heat come from current state")+
  scale_y_continuous(breaks=seq(-1500,1000,300))

plot_GWP 

ggsave("GWP1.jpg",plot_GWP,width = 7, height =2.5,dpi = 1200)
# create a dataframe that only include negative GWP
Action_negative = c(paste0("Action",2:8),paste0("Action",11:16),paste0("Action",20:21),paste0("Action",24:25))
...
HTT_Action_LCNC <- function(a){
  ...
  action_GWP = mean(HTT_Action_GWP(a))
...
Pyrolysis_Action_LCNC <- function(a){
  ...
  df$gas_oil_heat <- (df$HHV - df$biochar_HHV)*1000*0.85
  # if the heat created by  gas and oil is not sufficient, extra heat from natural gas should be used to supply for drying unit 
  ifelse(df$gas_oil_heat>df$drying_heat, drying_cost <-0, drying_cost <- (df$drying_heat - df$gas_oil_heat)*NG_cost*feedstock_annual/1000^2)
  # calculate variable costs
  cost_in <- feedstock_cost + pre_cost +drying_cost
  # calculate exported heat, heat was subsituted for the cost of NG
  out_heat <- ifelse(feedstock == "Biosolids", 0, ((df$HHV - df$biochar_HHV)*0.9*1000-df$drying_heat)*NG_cost*feedstock_annual/1000^2)
  ...
  action_GWP = mean(Pyrolysis_Action_GWP(a))
...
Gasification_Action_LCNC <- function(a){
  ...
  df$drying_heat <- 1000/(1-feedstock_parameters[feedstock_parameters$feedstock == feedstock,]$Moisture)*2.28/0.75
  # capital cost
  capital_cost_IGCC <- rtriangle(n=1000,a=2611*0.7,b=2611*1.3,c=2611)
  # fixed O&M cost; FOM = $170/kw year = (170*300*1000/1000^2)*541.7/550.8 = 50.2 M/year
  FOM_cost_IGCC <- rtriangle(n=1000,a=50.2*0.7,b=50.2*1.3,c=50.2)
  # vairable O&M cost; VOM = $18/Mwh  = 18*300*7000/1000^2*541.7/550.8= 37.2 M/year
  variable_IGCC <- rtriangle(n=1000,a=37.2*0.7,b=37.2*1.3,c=37.2)
  ...
  drying_cost <- feedstock_t_IGCC*(df$drying_heat)*NG_cost/1000^2
  # get GWP of handling 1 t feedstock
  action_GWP = mean(Gasification_Action_GWP(a))
...
Combustion_Action_LCNC <- function(a){
  ...
  # calculate heat required for drying 
  df$drying_heat <- 1000/(1-feedstock_parameters[feedstock_parameters$feedstock == feedstock,]$Moisture)*2.28/0.75
  # capital cost, 2010$ to 2016$
  capital_cost_PC <- rtriangle(n=1000,a=2271*0.7,b=2271*1.3,c=2271)
  # fixed O&M cost; F O &M = $116/kw year = 116*300*1000/1000^2*541.7/550.8 = 34.2 M/year
  FOM_cost_PC <- rtriangle(n=1000,a=34.2*0.7,b=34.2*1.3,c=34.2)
  # vairable O&M cost; V O &M = $13.4/Mwh  = 13.4*300*7000/1000^2*541.7/550.8= 27.7 M/year
  variable_PC <- rtriangle(n=1000,a=27.7*0.7,b=27.7*1.3,c=27.7)
  # add up fixed OM and variable OM
  ...
  # calculate cost for biomass drying
  drying_cost <- feedstock_t_PC*(df$drying_heat)*NG_cost/1000^2
  # get GWP of handling 1 t feedstock
  action_GWP = mean(Gasification_Action_GWP(a))
  # calculate the annual GWP of such action 
...
# plot boxplot diagram, extreme outliers are not shown. 
plot_LCNC <- ggplot() + geom_boxplot(data = kk,fatten =1, aes(x = reorder(variable,value, FUN = mean), y = value, fill = technology),outlier.shape = NA)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  ylab("Levelized cost of negative carbon ($/t CO2)")+theme(axis.text.x = element_text(angle = 90))+xlab("Action")+  
  stat_summary(fun=mean, colour="darkred", geom="point", 
                                                                                                                                                                                                                                                                                                                                                                                                                                     shape=18, size=3)+theme(legend.position= "none")+
  geom_text(data = means, aes(label = value, x = variable, y = 800))+
  ...
  scale_y_continuous(limits = c(-400, 800),breaks=seq(-600,1200,300))

  

plot_LCNC

ggsave("LCNC.jpg", plot_LCNC, height = 3, width = 7,dpi = 1200)



plot_LCNC_sensitivity <- ggplot(data = kk) + geom_histogram(alpha=0.6, binwidth = 20, position = "identity",color = "black", aes(x = value, fill = variable))+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) + ylab("Levelized cost of carbon ($/t CO2)")+theme(axis.text.x = element_text(angle = 90))+xlab("Action")+theme(legend.position= "none")+
  ggtitle("")+
  theme(plot.title = element_text(size=10,face = "bold"), axis.title.y = element_text(size = 8, face = "bold"),
        axis.title.x = element_blank(),axis.text = element_text(size = 6,face = "bold"))+facet_wrap(~variable,scales='free')

plot_LCNC_sensitivity

ggsave("LCNC_sensitivity.jpg",plot_LCNC_sensitivity, height = 6, width = 7,dpi = 1200)
