...
gwp_electricity <- 30/1000 #kg CO2/KWh
...
ggsave("GWP_ZERO.jpg", plot_GWP_zero,height = 2.5, width = 7,dpi = 1200)
...
Combustion_Action_LCNC <- function(a){
  ...
  # calculate heat required for drying 
  df$drying_heat <- 1000/(1-feedstock_parameters[feedstock_parameters$feedstock == feedstock,]$Moisture)*2.28/0.75
  # capital cost, convert 2010$ to 2016$
...
plot_LCNC_ZERO <- ggplot() + geom_boxplot(data = kk,fatten =1, aes(x = reorder(variable,value, FUN = mean), y = value, fill = technology),outlier.shape = NA)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ylab("Levelized cost of negative carbon ($/t CO2)")+
  theme(axis.text.x = element_text(angle = 90))+xlab("Action")+  stat_summary(fun=mean, colour="darkred", geom="point", shape=18, size=2)+theme(legend.position= "bottom")+
  ...
  scale_y_continuous(breaks=seq(-1500,1200,300))
...
ggsave("LCNC_Zero_sensitivity.jpg",plot_LCNC_sensitivity, height = 6, width = 7,dpi = 1200)
